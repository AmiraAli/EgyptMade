...
<div class="comments_section">
    ...
    <div id="comments" class="comments-area">
        ...
        if (function_exists('pll_e')) {
            ...
            $leavemsg=pll__('Leave a Reply');
            $logIn=pll__('Logged in as');
            $logOut=pll__('Log out');


        } else {
            $message= _e('MESSAGE', 'woocommerce');
            $leavemsg= _e('Leave a Reply', 'woocommerce');
            $logIn= _e('Logged in as', 'woocommerce');
            $logOut= _e('Log out', 'woocommerce');
        ...
        $getbowtied_comments_args = array(
            ...
            'title_reply' => esc_html__( $leavemsg, 'getbowtied' ),
            ...
            'comment_notes_after'  => '',
            'logged_in_as'  =>  '<p class="logged-in-as">' . sprintf( wp_kses( __( '"'.$logIn.' "<a class="user" href="%1$s">%2$s</a>. <a class="logout" href="%3$s" title="Log out of this account"><i class="fa fa-power-off"></i> '.'"'.$logOut.'"?</a>', 'getbowtied' ), $allowed_html_array ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>'
