...
if (function_exists('pll__')) {
    $label = pll__('Orders for');
} else {
    $label = 'Orders for';
}
?>
<h2><?php printf('%s %s', $label, get_product($product_id)->get_title()); ?></h2>

<table class="table table-striped table-bordered">
    <thead>
        <tr>
            <?php foreach ($headers as $header) : ?>
                <th class="<?php echo sanitize_title($header); ?>"><?php
                    if (function_exists('pll__')) {
                        echo pll__($header);
                    } else {
                        echo $header;
    ...
    <tbody>
        ...
        foreach ($body as $order_id => $order) :
            ...
            <tr>
                ...
                foreach ($order as $detail_key => $detail) : if ($detail_key == $last_index)
                        continue;
                    ?>
                ...
                <td colspan="100%">
                    ...
                    if ($can_view_comments || $can_add_comments) :
                        ...
                        <a href="#" class="order-comments-link">
                            <p>
                                <?php printf(__('%s (%s)', 'wcvendors'),$label, count($comments)); ?>
                            </p>
                        </a>
