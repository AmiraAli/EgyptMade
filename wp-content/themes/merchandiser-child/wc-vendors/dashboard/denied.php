...
<?php if (WCV_Vendors::is_pending(get_current_user_id())) { ?>
    ...
    <p>
        ...
        if (function_exists('pll_e')) {
            pll_e('ot yet been approved to become a vendor.  When it is, you will receive an email telling you that your account is approved!');
        } else {
...
<?php } else { ?>
    ...
    <p>
        <?php
        if (function_exists('pll_e')) {
            pll_e('Your account is not setup as a vendor.');
        } else {
            _e('Your account is not setup as a vendor.', 'wcvendors');
        }
        ?>
    </p>
            ...
            <p class="form-row">
                <input class="input-checkbox"
                       ...
                       if (function_exists('pll__')) {
                           $apply_vendor_string = pll__('Apply to become a vendor?');
                       } else {
                           $apply_vendor_string = __('Apply to become a vendor? ', 'wcvendors');
                       }
                       ?>
                <label for="apply_for_vendor"
                       class="checkbox"><?php echo apply_filters('wcvendors_vendor_registration_checkbox', $apply_vendor_string); ?></label>
            </p>
